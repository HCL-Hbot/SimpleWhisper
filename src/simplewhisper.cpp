...
SimpleWhisper::SimpleWhisper(simplewhisper_model_params params) {
    ...
    m_ctx_wsp = whisper_init_from_file_with_params(params.model_wsp.c_str(), m_cparams);
    if (!m_ctx_wsp) {
...
void SimpleWhisper::init() {
    ...
    const std::string prompt_whisper = ::replace(k_prompt_whisper, "{1}", m_params.bot_name);
}


std::string SimpleWhisper::do_inference(std::vector<float> &audio_samples) {
    std::string all_heard;
    all_heard = ::trim(::transcribe(m_ctx_wsp, m_params, audio_samples, k_prompt_whisper, m_prob0, m_t_ms));
