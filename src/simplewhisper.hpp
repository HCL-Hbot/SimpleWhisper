...
public:
   ...
   *
   * @param params A struct containing configuration for model inference;
   * Runtime as well as behavioural settings
   */
  SimpleWhisper(simplewhisper_model_params params);

  /**
   * @brief Initialize whisper by initializing a talk session with a initializer
   * response
   */
  void init();
